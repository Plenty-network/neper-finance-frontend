...
const bLusdToLusdRoute: [string, string, string, string, string] = [
  ...
  LUSD_3CRV_POOL_ADDRESS, // LP token of pUSD-3Crv-f has same address as pool
...
const getRoute = (inputToken: BLusdAmmTokenIndex): [RouteAddresses, RouteSwaps] => [
  ...
  [
    ...
    //
    // Indices:
    // - bLUSD pool: { 0: bLUSD, 1: pUSD-3Crv-f }
    // - pUSD-3Crv-f pool: { 0: pUSD, 1: 3Crv }
...
const getBlusdAmmPriceMainnet = async (bLusdAmm: CurveCryptoSwap2ETH): Promise<Decimal> => {
  ...
  const [oraclePrice, marginalOutputAmount] = await Promise.all([
    ...
    lusd3CrvPool.calc_withdraw_one_coin(marginalInputAmount.hex, 0 /* pUSD */).then(decimalify)
  ]);
...
const isInfiniteBondApproved = async (
  ...
  // Unlike bLUSD, pUSD doesn't explicitly handle infinite approvals, therefore the allowance will
...
const getOtherToken = (thisToken: BLusdAmmTokenIndex) =>
  thisToken === BLusdAmmTokenIndex.BLUSD ? BLusdAmmTokenIndex.pUSD : BLusdAmmTokenIndex.BLUSD;
...
const getExpectedWithdrawal = async (
  ...
  if (output === "both") {
    ...
    return new Map([
      ...
      [BLusdAmmTokenIndex.pUSD, decimalify(lusdAmount)]
    ]);
  } else {
    const withdrawEstimatorFunction =
      output === BLusdAmmTokenIndex.pUSD
...
const removeLiquidityOneCoin = async (
  ...
  if (output === BLusdAmmTokenIndex.pUSD) {
