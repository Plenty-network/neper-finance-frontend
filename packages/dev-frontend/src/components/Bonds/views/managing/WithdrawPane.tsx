...
import { DisabledEditableAmounts, DisabledEditableRow, EditableRow } from "../../../Vault/Editor";
...
const checkOutput = (value: string): BLusdAmmTokenIndex | "both" => {
  ...
  if (i === BLusdAmmTokenIndex.BLUSD || i === BLusdAmmTokenIndex.pUSD) {
    return i;
  }
...
const zeros = new Map<BLusdAmmTokenIndex, Decimal>([
  ...
  [BLusdAmmTokenIndex.pUSD, Decimal.ZERO]
]);
...
export const WithdrawPane: React.FC = () => {
  ...
  const handleConfirmPressed = () => {
    ...
    if (output === "both") {
      ...
      const minLusdAmount = withdrawal.get(BLusdAmmTokenIndex.pUSD)?.mul(1 - curveSlippage);
