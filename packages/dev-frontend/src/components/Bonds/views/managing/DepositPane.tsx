...
import { DisabledEditableRow, EditableRow } from "../../../Vault/Editor";
...
export const DepositPane: React.FC = () => {
  ...
  const handleApprovePressed = () => {
    ...
    if (zapperNeedsLusdApproval) {
      tokensNeedingApproval.set(BLusdAmmTokenIndex.pUSD, addresses.BLUSD_LP_ZAP_ADDRESS);
    }
  ...
  const handleSetAmount = (token: "bLUSD" | "pUSD", amount: Decimal) => {
    if (shouldDepositBalanced) {
      if (token === "bLUSD") setLusdAmount(poolBalanceRatio.mul(amount));
      else if (token === "pUSD") setBLusdAmount(amount.div(poolBalanceRatio));
    }

    if (token === "bLUSD") setBLusdAmount(amount);
    else if (token === "pUSD") setLusdAmount(amount);
  };
  ...
  return (
    <>
      ...
      <EditableRow
        ...
        setEditedAmount={amount => handleSetAmount("pUSD", Decimal.from(amount))}
          ...
          <InfoIcon
            ...
            tooltip={
              <Card variant="tooltip">
                Tick this box to deposit bLUSD and pUSD-3CRV in the pool's current liquidity ratio.
                Current ratio = 1 bLUSD : {poolBalanceRatio.prettify(2)} pUSD.
              </Card>
            }
          />
        </Flex>
      ...
      {isAnyBalanceInsufficient && (
        <ErrorDescription>
          ...
          {isLusdBalanceInsufficient && (
            <Amount>{lusdAmount.sub(coalescedLusdBalance).prettify(2)} pUSD</Amount>
          )}
        </ErrorDescription>
      )}
