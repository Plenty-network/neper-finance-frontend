...
const getBondEvents = (bond: OptimisticBondType): EventType[] => {
  return [
    ...
    },
    {
      ...
      label: (
        <>
          <Label description="How many bLUSD are recommended before claiming the bond, selling the bLUSD for pUSD, and then opening another bond.">
      ...
      <Card mt={[0, 0, 0, 0]} sx={{ borderRadius: 12, flexGrow: 1 }}>
        <Flex p={[2, 3]} sx={{ flexDirection: "column" }}>
          ...
          <Flex mt={4} variant="layout.actions" sx={{ justifyContent: "flex-end" }}>
            ...
            >
              <Record lexicon={l.BOND_DEPOSIT} value={bond.deposit.prettify(2)} type="pUSD" />

              <Record lexicon={l.MARKET_VALUE} type="pUSD" />
            </Flex>
