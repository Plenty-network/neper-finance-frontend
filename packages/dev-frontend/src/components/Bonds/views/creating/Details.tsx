  ...
  return (
    <>
      <Heading as="h2" sx={{ pt: 1, pb: 3, px: 2 }}>
        <Flex sx={{ justifyContent: "center" }}>Bond pUSD</Flex>
        <Close
      ...
      {!isInfiniteBondApproved && (
        <ActionDescription>
          <Text>You are approving pUSD for bonding</Text>
        </ActionDescription>
      )}

      {statuses.APPROVE === "FAILED" && (
        <Warning>Failed to approve spend of pUSD. Please try again.</Warning>
      )}
      ...
      {isInfiniteBondApproved && (
        <ActionDescription>
          You are bonding <Text sx={{ fontWeight: "bold" }}>{deposit.prettify(2)} pUSD</Text>
        </ActionDescription>
      )}

      {!isDepositEnough && <ErrorDescription>The minimum bond amount is 100 pUSD.</ErrorDescription>}
      {doesDepositExceedBalance && (
        <ErrorDescription>
          Amount exceeds your balance by <Amount>{deposit.sub(lusdBalance).prettify(2)} pUSD</Amount>
        </ErrorDescription>
      )}

      <Flex pb={2} sx={{ fontSize: "15.5px", justifyContent: "center", fontStyle: "italic" }}>
        You can cancel your bond at any time to recover your deposited pUSD
      </Flex>
