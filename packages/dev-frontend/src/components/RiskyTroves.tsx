...
const liquidatableInRecoveryMode = (
  ...
  if (collateralRatio.gte(MINIMUM_COLLATERAL_RATIO) && collateralRatio.lt(totalCollateralRatio)) {
    return [
      ...
      "There's not enough pUSD in the Stability pool to cover the debt"
    ] as const;
  } else {
...
export const RiskyTroves: React.FC<RiskyTrovesProps> = ({ pageSize }) => {
  ...
  return (
    <Card sx={{ width: "100%" }}>
      <Heading>
        <Abbreviation short="Vaults">Risky Vaults</Abbreviation>
            ...
            <thead>
              <tr>
                ...
                <th>
                  ...
                  <Box sx={{ fontSize: [0, 1], fontWeight: "body", opacity: 0.5 }}>WBTC</Box>
                </th>
                <th>
                  Debt
              ...
              {troves.map(
                trove =>
                  !trove.isEmpty && ( // making sure the Vault hasn't been liquidated
                    // (WONT-FIX: remove check after we can fetch multiple Vaults in one call)
