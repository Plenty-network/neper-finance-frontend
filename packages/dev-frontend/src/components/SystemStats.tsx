...
const Balances: React.FC = () => {
  ...
  return (
    <Box sx={{ mb: 3 }}>
      ...
      <Statistic lexicon={l.WBTC}>{accountBalance.prettify(4)}</Statistic>
      <Statistic lexicon={l.pUSD}>{lusdBalance.prettify()}</Statistic>
...
export const SystemStats: React.FC<SystemStatsProps> = ({ variant = "info", showBalances }) => {
  ...
  return (
    <Card {...{ variant }}>
      ...
      <Heading>Neper Finance statistics</Heading>
      ...
      <Statistic lexicon={l.TVL}>
        {total.collateral.shorten()} <Text sx={{ fontSize: 1 }}>&nbsp;WBTC</Text>
        <Text sx={{ fontSize: 1 }}>
          &nbsp;(${Decimal.from(total.collateral.mul(price)).shorten()})
        </Text>
      </Statistic>
      <Statistic lexicon={l.DAILY_MCR_CHANGE}>{"0%"}</Statistic>
