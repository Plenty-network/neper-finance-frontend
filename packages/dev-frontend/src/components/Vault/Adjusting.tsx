...
const applyUnsavedCollateralChanges = (unsavedChanges: Difference, trove: Vault) => {
...
const applyUnsavedNetDebtChanges = (unsavedChanges: Difference, trove: Vault) => {
  if (unsavedChanges.absoluteValue) {
    if (unsavedChanges.positive) {
...
export const Adjusting: React.FC = () => {
  ...
  const previousTrove = useRef<Vault>(trove);
  ...
  const updatedTrove = isDirty ? new Vault(collateral, totalDebt) : trove;
        ...
        <StaticRow
          ...
          infoIcon={
            <InfoIcon
              tooltip={
                <Card variant="tooltip" sx={{ width: "240px" }}>
                  The total amount of pUSD your Vault will hold.{" "}
                  {isDirty && (
                    <>
                      You will need to repay {totalDebt.sub(LUSD_LIQUIDATION_RESERVE).prettify(2)}{" "}
                      pUSD to reclaim your collateral ({LUSD_LIQUIDATION_RESERVE.toString()} pUSD
